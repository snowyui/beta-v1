...
export function serializeStyle<T extends NonNestObjectType>(object: T) {
  ...
  const selector = (dot: string, colon: string, property: string, rule: string, space?: string) => {
    return dot + base62Hash + colon + property + ' {\n' + rule + space;
  };
    ...
    if (property.startsWith('@media')) {
      ...
      for (const mediaProp in value) {
        ...
        if (pseudo.classes.includes(mediaProp)) {
          ...
          mediaRule += selector('  .', ':', mediaProp, pseudoClassRule, '  }  ' + endNoLine);
        } else if (pseudo.elements.includes(mediaProp)) {
          let pseudoElementRule = '';
          for (const pseudoProp in mediaValue) {
            pseudoElementRule += rules('    ', mediaValue, pseudoProp);
          }
          mediaRule += selector('  .', '::', mediaProp, pseudoElementRule, '  }  ' + endNoLine);
        } else {
  ...
  if (isInDevelopment) console.log('.....💫 Melt Scoped CSS style .....' + styleSheet);
